...
class LocalStore {
  ...
  static const _walletsKey = 'mahfazty_wallets_v1';
  static const _walletSourcesKey = 'mahfazty_wallet_sources_v1';
  static const _schemaVersionKey = 'mahfazty_schema_version';
  ...
  /// I4: حفظ زوج أحداث دفعة واحدة (SharedPreferences write واحد = atomic).
  /// ده بيضمن إن التحويل الداخلي بيتسجل الجزأين أو لا شيء.
  Future<void> saveEventPair(FinancialEvent a, FinancialEvent b) async {
    final events = await loadEvents();
    for (final e in [a, b]) {
      final idx = events.indexWhere((x) => x.id == e.id);
      if (idx == -1) { events.add(e); } else { events[idx] = e; }
    }
    await _saveAllEvents(events);
  }

  /// I4: حذف زوج أحداث (atomic).
  Future<void> deleteEventPair(String idA, String idB) async {
    final events = await loadEvents();
    events.removeWhere((x) => x.id == idA || x.id == idB);
    await _saveAllEvents(events);
  }
  ...
  Future<void> setSchemaVersion(int version) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(_schemaVersionKey, version);
  }
  ...
  Future<void> saveWallets(List<Wallet> wallets) async {
    ...
    await prefs.setString(
        _walletsKey, jsonEncode(wallets.map((w) => w.toJson()).toList()));
  }

  Future<void> upsertWallet(Wallet wallet) async {
  ...
  Future<List<WalletSource>> loadWalletSources() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_walletSourcesKey);
    if (raw == null) return [];
    final list = jsonDecode(raw) as List;
    return list
        .map((e) => WalletSource.fromJson(e as Map<String, dynamic>))
        .toList();
  }

  Future<void> saveWalletSources(List<WalletSource> sources) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
        _walletSourcesKey,
  ...
  Future<void> upsertWalletSource(WalletSource source) async {
    final sources = await loadWalletSources();
    ...
    await saveWalletSources(sources);
  }

  Future<void> deleteWalletSource(String id) async {
    final sources = await loadWalletSources();
