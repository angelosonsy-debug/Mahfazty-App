...
class SmsParseResult {
  ...
  /// I8: شرح مبسّط للمستخدم — كيف تم تفسير الرسالة (للـ Source Test UI)
...
class SmsParser {
  ...
  static SmsParseResult parse(String sender, String body, {DateTime? receivedAt}) {
    ...
    for (final template in templates) {
      ...
      if (match != null) {
        ...
        return SmsParseResult(
          ...
          explanation: 'تم التعرف على النمط «${template.id}» من المصدر $sender.',
        );
      }
    }
    ...
    final explainFallback = 'تم التعرف على الرسالة من السياق العام. المبلغ: '
        '\${fallbackEvent.amount?.toStringAsFixed(2) ?? "غير محدد"}';
    return SmsParseResult(
      matchedSource: matchedSource,
      event: fallbackEvent,
      normalizedBody: normalized,
