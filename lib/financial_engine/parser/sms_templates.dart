...
typedef SmsTemplateBuilder = TemplateExtraction Function(RegExpMatch match);

class SmsTemplate {
  final String id;
  final RegExp pattern;
  final SmsTemplateBuilder build;
...
final List<SmsTemplate> vodafoneCashTemplates = [
  ...
  // I5 — تم خصم: النمط العام غير المغطى بالـtemplates المخصصة أعلاه.
  // Source = VF-Cash (التحقق يحصل في SourcePolicy قبل أي template)،
  // فثقة 92 مناسبة — أعلى من الـ semantic fallback (85)، أقل من الكامل (99).
  SmsTemplate(
    id: 'vf_debit_general',
    pattern: RegExp(
      r'تم خصم (?:مبلغ )?(?<amount>\d+(?:\.\d+)?) جني[ةه]'
      r'(?:.*?رصيد (?:حسابك|محفظتك) الحالي (?<balance>\d+(?:\.\d+)?) جني[ةه])?'
      r'(?:.*?رقم العملية (?<ref>\d+))?',
      dotAll: true,
    ),
    build: (m) => TemplateExtraction(
