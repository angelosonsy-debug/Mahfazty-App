...
void main() {
  ...
  group('سياسة InstaPay: السمة الأهلي بس هي اللي بتحدّث الرصيد', () {
    ...
    test('رسالة بنك أهلي من غير رصيد صريح بتضيف/تخصم من آخر رصيد معروف', () async {
      ...
      await engine.setWalletBalanceManuallyBySource(FinancialSource.instaPay, 1000);
    ...
    test('حدث مصدره InstaPay مباشرة بثقة عالية (زي بعد تأكيد يدوي) بيأثر على الرصيد', () async {
      ...
      await engine.setWalletBalanceManuallyBySource(FinancialSource.instaPay, 500);
  ...
  group('Manual editing', () {
    ...
    test('تعديل رصيد محفظة كامل يدويًا', () async {
      ...
      await engine.setWalletBalanceManuallyBySource(FinancialSource.vodafoneCash, 750);
  ...
  group('استمرارية تعديل رصيد المحفظة عبر إعادة التشغيل (Bug Fix)', () {
    test('رصيد أولي -> تعديل يدوي -> إعادة تحميل -> القيمة المعدّلة تفضل', () async {
      ...
      // رصيد أولي
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.vodafoneCash, 1000);
      expect(engine1.walletBalance[FinancialSource.vodafoneCash], 1000);

      // تعديل يدوي لاحق
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.vodafoneCash, 2500);
    ...
    test('تعديل مرتين -> إعادة تحميل -> القيمة التانية هي اللي بتفضل', () async {
      ...
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.instaPay, 300);
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.instaPay, 800);
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.instaPay, 150);
    ...
    test('الحسابات (صافي الثروة) بتستخدم آخر قيمة محفوظة بعد إعادة التحميل', () async {
      ...
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.vodafoneCash, 500);
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.vodafoneCash, 3000);
    ...
    test('نفس السلوك لما يكون فيه أحداث تلقائية (SMS) قبل التعديل اليدوي', () async {
      ...
      await engine1.setWalletBalanceManuallyBySource(FinancialSource.vodafoneCash, 5000);
