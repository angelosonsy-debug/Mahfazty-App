...
void main() {
  ...
  group('SourcePolicy - InstaPay: مقبولة بس من تطبيق InstaPay نفسه (بالـ package name)', () {
    test('package name فيه instapay - مقبول', () {
      expect(SourcePolicy.isInstaPayAppNotification('com.instapay.egypt'), isTrue);
      expect(SourcePolicy.isInstaPayAppNotification('com.example.InstaPay'), isTrue);
    });

    test('نص الإشعار مش كافي - لازم الـ package name نفسه', () {
      // ده بالظبط الباگ اللي كان بيحصل: نص فيه "InstaPay" من تطبيق تاني
      // خالص - المفروض يترفض لأن التحقق بقى بالـ package name مش بالنص
      expect(SourcePolicy.isInstaPayAppNotification('com.whatsapp'), isFalse);
      expect(SourcePolicy.isInstaPayAppNotification('com.vodafone.vodafonecash'), isFalse);
      expect(SourcePolicy.isInstaPayAppNotification('com.android.systemui'), isFalse);
      expect(SourcePolicy.isInstaPayAppNotification('com.some.random.app'), isFalse);
    });
  });
}
